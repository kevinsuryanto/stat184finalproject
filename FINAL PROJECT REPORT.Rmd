...
#Research Question
What affects an NBA player's salary? What are the biggest contributing factors to a player's salary? Does more playing time result in higher salary?
...
```
According to figure 1.2, higher points result in higher salary amongst all player positions. Furthermore, we will observe how other key NBA stats affect a player's salary.
...
```
We can obseve that there is a slight upward trend in salary in terms of blocks per game although it is not significant.
...
Linear regression models are used to show or predict the relationship between two variables or factors. We will fit salary as the response variable and use number of points, assists, rebounds, steals, blocks, and minutes played as the predictors.
```{r}
model <- lm(SALARY~POINTS+AST+TRB+STL+BLK+MP)
summary(model)
```
From the p-value results above, we are 95% confident that number of points, assist, and rebound have a significant relationship with a player's salary. However, we fail to reject the null hypothesis that steals, blocks, and minutes player significantly affect a player's salary.

#Conclusion
From the graphical and linear regression observations, we can conclude that the number of points per game by an NBA player significantly affects an NBA player's salary. Moreover, other key important factors such as number of assists and rebounds also significantly affect a player's salary. Some people may say that more playing time (or minutes played) have higher salaries. However, although our graph shows that there is a positive relationship between minutes played and salary, the linear regression output shows that the effect of minutes played is not significant to salary.
